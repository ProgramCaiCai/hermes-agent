...
class GatewayRunner:
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if _quick_key in self._running_agents and _stale_ts:
            ...
            if _should_evict:
                logger.warning(
                    ...
                    _quick_key[:30], _stale_age, (_stale_idle if _stale_idle is not None else -1),
        ...
        if _quick_key in self._running_agents:
            ...
            # /background and /spawn must bypass the running-agent guard.
            # They start parallel work and must never interrupt the active
            # conversation. /spawn also needs the latest in-memory snapshot.
