...
def _to_openai_base_url(base_url: str) -> str:
    ...
    completions. The auxiliary client uses the OpenAI SDK for chat/responses
    modes, so it must hit the ``/v1`` surface.
    """
...
def _build_explicit_custom_client(
    *,
    ...
    if resolved_mode == "codex_responses":
        real_client = OpenAI(api_key=api_key, base_url=_to_openai_base_url(base_url), **extra)
    ...
    return OpenAI(api_key=api_key, base_url=_to_openai_base_url(base_url), **extra), model
...
def resolve_provider_client(
    ...
    try:
        ...
        if custom_entry:
            ...
            custom_api_mode = _normalize_api_mode(custom_entry.get("api_mode"))
            if custom_base:
                final_model = _normalize_resolved_model(
                ...
                logger.debug(
                    ...
                    provider, final_model, custom_api_mode or "chat_completions")
                return (_to_async_client(client, final_model) if async_mode
                        else (client, final_model))
            logger.warning(
...
def get_vision_auxiliary_client(*, async_mode: bool = False) -> Tuple[Optional[Any], Optional[str]]:
    """Backward-compatible wrapper returning just (client, model) for vision."""
    _provider, client, model = resolve_vision_provider_client(async_mode=async_mode)
    return client, model
...
def _resolve_forced_provider(
    ...
    *,
    ...
    """Backward-compatible wrapper for explicit provider selection."""
    resolved_provider = _normalize_vision_provider(provider)
    resolved_model = model
    if resolved_provider == "custom" and not resolved_model:
