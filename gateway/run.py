        ...
        if _quick_key in self._running_agents and _stale_ts:
            ...
            if (
                ...
                try:
                    ...
                    pass
            elif _stale_agent is _AGENT_PENDING_SENTINEL:
                _stale_detail = " | pending startup"
            # Evict if: a real agent is idle beyond timeout, OR any entry (including
            # a leaked pending sentinel) exceeds an extreme wall-clock TTL.
            _wall_ttl = max(_raw_stale_timeout * 10, 7200) if _raw_stale_timeout > 0 else float("inf")
            _should_evict = (
                _stale_age > _wall_ttl
                or (
