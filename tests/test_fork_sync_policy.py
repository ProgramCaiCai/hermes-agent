...
def test_agents_declares_semantic_sync_as_default():
    ...
    assert "`archive/runtime-fixes` and `archive/custom-api-mode` are historical reference branches only." in text
...
def test_fork_sync_doc_forbids_mechanical_merge_as_default():
    ...
    assert "这些命令也不是默认流程" in text
    assert "只修复失败所指向的语义缺口" in text
    assert "当前确认需保留的活跃 fork 语义：`/spawn`" in text
    assert "当前确认已归档的吸收语义：`archive/runtime-fixes`、`archive/custom-api-mode`" in text
