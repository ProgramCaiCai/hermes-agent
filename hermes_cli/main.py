...
def _model_flow_named_custom(config, provider_info):
    ...
    Loopback endpoints keep the legacy fast path: if a model was previously
    saved, activate it immediately without probing again. Other endpoints
    still probe /models and let the user pick even when a model is saved.
    Falls back to manual entry when probing fails and no saved model exists.
    """
    from hermes_cli.auth import _save_model_choice, deactivate_provider
    from hermes_cli.config import load_config, save_config
    from hermes_cli.models import fetch_api_models
    ...
    def _activate_model(model_name: str) -> None:
        _save_model_choice(model_name)
    ...
    if saved_model and _is_loopback_endpoint(base_url):
        _activate_model(saved_model)
